    ...
    <RevealSection className="max-w-[800px] mx-auto px-5 sm:px-[5%]">
      ...
      <div className="rounded-2xl border border-white/[0.06] overflow-hidden bg-white/[0.015]">
        <div className="grid grid-cols-[1fr_90px_90px] sm:grid-cols-[1fr_120px_120px] text-xs sm:text-sm">
          ...
          <div className="px-3 py-3 text-orange-400 font-semibold text-center border-b border-white/[0.06] bg-orange-500/5">SalesFlow</div>
          ...
          {rows.map((row, i) => (
            <div key={i} className="contents">
              ...
              <div className={`px-3 py-3.5 flex items-center justify-center bg-orange-500/[0.03] ${i < rows.length - 1 ? "border-b border-white/[0.04]" : ""}`}>
                {row.sf === true ? <Check className="w-4 h-4 text-orange-400" /> : <span className="text-orange-400 font-semibold text-xs">{row.sf}</span>}
              </div>
