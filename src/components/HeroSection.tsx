...
const dashboardImages = [{
  src: dashboardVendas,
  alt: "Dashboard Vendas IA - Análise em tempo real",
  label: "Vendas"
}, {
  src: dashboardPipeline,
  alt: "Pipeline IA - Gestão de leads automatizada",
  label: "Pipeline"
}, {
  src: dashboardCampaigns,
  alt: "Disparador de Campanhas - Automação WhatsApp",
  label: "Campanhas"
}, {
  src: dashboardFollowup,
...
const HeroSection = () => {
  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden py-20 px-4">
      ...
      <div className="absolute bottom-20 right-10 w-96 h-96 bg-emerald/10 rounded-full blur-[120px] animate-float" style={{
      animationDelay: "1s"
    }} />
      ...
      <div className="container relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          delay: 0.2
            ...
            <Button size="lg" className="h-12 sm:h-14 px-4 sm:px-8 text-sm sm:text-lg font-bold bg-emerald hover:bg-emerald-glow text-background glow-emerald transition-all duration-300 group w-full sm:w-auto">
            ...
            <p className="text-sm text-muted-foreground mt-4">🔒 Pagamento único • Sem mensalidades</p>
          </motion.div>
          ...
          delay: 0.4
            ...
            <div className="relative">
              ...
              <div className="relative z-20 flex items-center justify-center">
                {/* Previous Button */}
                <button onClick={prevSlide} className="absolute left-0 md:left-4 z-30 p-2 md:p-3 rounded-full bg-card/80 border border-border hover:bg-card hover:border-violet/50 transition-all duration-300 backdrop-blur-sm group" aria-label="Imagem anterior">
                ...
                <div className="relative w-full max-w-[800px] mx-8 md:mx-16 overflow-visible rounded-xl">
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    ease: "easeOut"
                      ...
                      <img src={dashboardImages[currentIndex].src} alt={dashboardImages[currentIndex].alt} className="relative w-full rounded-xl border-2 border-emerald/40 shadow-2xl shadow-emerald/30" loading="eager" />
                      {/* Floating label */}
                      <motion.div initial={{
                      opacity: 0,
                      scale: 0.8
                ...
                {/* Next Button */}
                <button onClick={nextSlide} className="absolute right-0 md:right-4 z-30 p-2 md:p-3 rounded-full bg-card/80 border border-border hover:bg-card hover:border-violet/50 transition-all duration-300 backdrop-blur-sm group" aria-label="Próxima imagem">
              ...
              <div className="flex justify-center gap-2 mt-6">
                {dashboardImages.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentIndex ? "w-8 bg-emerald" : "bg-muted-foreground/30 hover:bg-muted-foreground/50"}`} aria-label={`Ir para imagem ${index + 1}`} />)}
              </div>
            </div>
          </motion.div>
        </div>
